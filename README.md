...
One source tree, three targets. `.chezmoiignore` decides which half of it lands
...
| --- | --- | --- |
| Linux desktop + laptop | Hyprland | the main setup — [below](#hyprland-specific) |
| MacBook Pro | AeroSpace | [macOS specific](#macos-specific) |
| WSL | none | shell half only: zsh, tmux, nvim — [WSL specific](#wsl-specific) |

The Linux half runs on Arch today and is **being moved to NixOS**, and then
home-manager. [NixOS migration](#nixos-migration) is what that has already
changed in here, and what is still Arch-shaped.

There was a fourth target: a Windows work laptop (`G3JC7G4`) running GlazeWM
out of `.glzr`. It is gone — the config, the two `windows` blocks in
`.chezmoiignore` and the notes here all went with it. `git log -- dot_glzr` has
it if it is ever wanted back.

**Everything lives on the `hyprland` branch, not `main`.** `main` is far
behind and has no macOS support at all, so clone with
...
`bootstrap.sh` works out which target it is on and runs three steps, in this
order:
...
**On NixOS step 1 is skipped**, because packages there come from the system
flake rather than from a script — steps 2 and 3 are unchanged. See
[NixOS migration](#nixos-migration).
...
`nixos` is the one answer `lib.sh` gives that `.chezmoiignore` does not need,
and that asymmetry is deliberate — see below.
...
The Linux half of this repo is moving from Arch to NixOS, and after that to
home-manager. None of it is generated by nix yet — this is still a chezmoi
tree — but the Arch-only assumptions that would have broken on the first boot
are out, and this is what changed.

**Four hardcoded `/usr` paths are gone.** Each was a *silent* failure rather
than an error, which is what made them worth finding before the move and not
during it:

| was | now | cost of leaving it |
| --- | --- | --- |
| `hypr/autostart.lua` → `/usr/lib/hyprpolkitagent/hyprpolkitagent` | `systemctl --user start hyprpolkitagent` | no polkit agent, so every authentication dialog in the session simply never appears |
| `uwsm/env` → `SUDO_ASKPASS`, `SSH_ASKPASS=/usr/bin/ksshaskpass` | `command -v` over a preference list, at login | no graphical sudo prompt, and FIDO2 PIN requests fail as "incorrect passphrase" |
| `environment.d/10-ssh-agent.conf` → the same path | `lookPath` at apply time; the file is a `.tmpl` now | ssh-agent cannot ask for a PIN, so signing fails with "agent refused operation" |
| `dbus-1/services/org.fcitx.Fcitx5.service` → `Exec=/usr/bin/fcitx5` | deleted | **the worst of the four.** It *overrode* the packaged service, so it would not have cost an icon, it would have cost the input method entirely |

The askpass helper is resolved at login in `uwsm/env` (shell, so it can look
things up) and at apply time in `environment.d` (`KEY=VALUE`, so it cannot) —
which is why only one of the two is a template. Install the helper *before*
applying, and re-apply if you change which one is installed.

That same file picks the agent socket name from `.chezmoi.osRelease`:
`%t/ssh-agent` on NixOS, `%t/ssh-agent.socket` everywhere else. **Keep the file
on NixOS** — `programs.ssh.startAgent` does export `SSH_AUTH_SOCK`, but out of
`environment.extraInit`, which is `/etc/profile` and so reaches shells only,
never the systemd user manager this file exists to feed.

`ksshaskpass` was also never in `install/arch.sh` despite two config files
naming it. It is now, along with the fcitx5 packages.

**The fcitx5 overrides are deleted rather than ported.**
`i18n.inputMethod.enable = true` with `type = "fcitx5"` does the D-Bus
activation properly, and a hand-written override in `$XDG_DATA_HOME` would
only fight it. The tray icon those two files existed to suppress is handled
from waybar's side now — see [Tray](#tray).

**`install/` knows about NixOS** in the only way that makes sense: it skips the
package step (packages are declarative; a shell script has nothing to add) and
runs the other two, which are identical everywhere. `common.sh` also stops
trying to `chsh` there — the passwd entry comes from
`users.users.<name>.shell`, so chsh either refuses or is reverted by the next
rebuild.

**`.chezmoiignore` deliberately does not know about NixOS.** A NixOS desktop is
a Linux desktop and wants the identical config set; only packaging differs. The
one place that needed a new answer is `dotfiles_target` in `install/lib.sh`,
which returns `nixos` for `/etc/NIXOS` — tested *after* WSL, because NixOS
under WSL is still WSL and still wants only the shell half.

**Still Arch-shaped, waiting on the migration proper:**

| thing | note |
| --- | --- |
| `system/greetd`, `system/keyring`, `system/logind` | `pacman -S` plus writes into `/etc`. All three become nix options (`services.greetd`, `services.gnome.gnome-keyring`, `services.logind`) — do **not** run these scripts on NixOS |
...
the shell half of this repo, so it lands on every target — and `common.sh`
...
`ksshaskpass` and the fcitx5 group are in
[`install/arch.sh`](install/arch.sh) but not in the line above, because both
are pulled in by config rather than by a keybind: the askpass helper is what
`uwsm/env` and `environment.d` look for (nothing else installs one, and without
one there is no graphical password prompt at all), and fcitx5 is
[its own section](#input-method-fcitx5).
...
out of `ksshaskpass`, `lxqt-openssh-askpass` and a generic `ssh-askpass`,
rather than naming a path. One of them has to be installed; `arch.sh` installs
the first.
...
| --- | --- | --- |
| `.config/environment.d/10-ssh-agent.conf` (source: `.tmpl`) | the systemd user manager, so every graphical app and user unit | a shell never reads environment.d |
...
```bash
SSH_ASKPASS="$(command -v ksshaskpass)" SSH_ASKPASS_REQUIRE=force ssh-keygen -K
```

(`command -v` rather than a literal path on purpose — see
[NixOS migration](#nixos-migration); the helper is not in the same place, or
even the same package, on every machine.)
...
(`install/arch.sh` installs that group too. On NixOS it is
`i18n.inputMethod.enable = true` with `type = "fcitx5"` and the engines in
`fcitx5.addons` instead — which is also what now owns the D-Bus activation
this repo used to override; see [Tray](#tray).)
...
Arch under WSL2 on a Windows work laptop (`G3JC7G4`). Only the shell half
lands: zsh, tmux, nvim, and the tools they shell out to. No compositor, because
the desktop over here *is* Windows.

### What .chezmoiignore drops

WSL is detected on the kernel release string containing `microsoft`, not on a
hostname, so it holds for any distro under any WSL install — a NixOS under WSL
included, which is the case that keeps this test worth having now that the
GlazeWM target it originally guarded against is gone.
